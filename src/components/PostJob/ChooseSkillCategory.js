        ...
        <div className="skill-subcat" style={requestForPostJob.category.name ? {} : { display: 'none' }}>
          <div className="flexbox justify-space-between form-block-wrapper">
                    ...
                    ? requestForPostJob.category.skill_categories.map((subSkill, index) => (
                        <div className="checkbox-block" key={index}>
                          <input
                            ...
                            name=""
                            onChange={() => {
                              chooseSkillCategory(index, requestForPostJob.category.skill_categories);
                            }}
            ...
            <div className="skill-sub-block">
              ...
              <ul
                style={
                  skill_tags.skill_tags.length && skill_tags.dropDownMenu
                    ? {
              ...
              <div className="skill-tags-block clearfix">
                {requestForPostJob.skill_tags[0]
                  ? requestForPostJob.skill_tags.map((skill_tag, index) => (
                      <div
                        className="skill-tag"
                        key={index}
                        onClick={() => {
                          deleteSkillTag(requestForPostJob.skill_tags, index);
                        }}
                      >
                        {skill_tag.name}
                      </div>
